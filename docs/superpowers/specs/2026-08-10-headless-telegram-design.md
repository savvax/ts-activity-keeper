...
| `src/launch-agent.js` | попытка прописать автозапуск + проверка результата | `test/launch-agent.test.js` |
...
Автозапуск: вкл | не удалось (<причина>) | выкл
...
- старт приложения (в том числе после ребута/автозапуска) — со строкой о
  результате попытки автозапуска и статусе keep-awake;
...
старте приложения и при каждом изменении настройки. Подробности — ниже.

## Автозапуск: попытка и отчёт

`src/launch-agent.js`:

```js
ensureAutostart({ app, desired })  // → { desired, actual, ok, reason }
```

Алгоритм при каждом старте приложения (если `settings.autostart === true`):

1. Прочитать `app.getLoginItemSettings()`. Если `openAtLogin` уже `true` —
   ничего не делать, вернуть `{ ok: true, reason: 'already' }`.
2. Иначе вызвать `app.setLoginItemSettings({ openAtLogin: true })` в
   try/catch.
3. Перечитать `app.getLoginItemSettings()` и сравнить с желаемым — это
   единственная надёжная проверка, потому что `setLoginItemSettings` ничего не
   возвращает и молча не срабатывает при отсутствии прав.
4. Если `openAtLogin` так и остался `false` — вернуть `{ ok: false }` с
   причиной.

Отдельно определяется **translocation / запуск не из `/Applications`**:
`app.getPath('exe')` внутри `/private/var/folders/` (App Translocation) или вне
`/Applications` означает, что зарегистрированный путь автозапуска будет
временным или пропадёт. Это считается `ok: false` с причиной `location`, даже
если сам вызов прошёл: пользователю нужно перетащить `.app` в `/Applications`
и запустить оттуда.

Причины (`reason`): `already`, `set`, `denied` (исключение или значение не
применилось), `location` (translocated / не в `/Applications`).

Ошибка автозапуска **не блокирует** приложение — трекинг и бот работают дальше.

### Сообщение при старте

Стартовое сообщение в привязанный чат всегда включает строку про автозапуск:

```
▶️ TS Activity Keeper запущен (v0.0.4)
Трекинг: стартует | ожидает /login
Автозапуск: включён | не удалось включить — <причина и что сделать>
Keep-awake: активен
```

Тексты причин:

- `denied` — «система не дала прописать автозапуск; разреши приложение в
  Системных настройках → Основные → Объекты входа»;
- `location` — «приложение запущено не из /Applications — перетащи его в
  /Applications и запусти снова, иначе автозапуск не сохранится».

Если чат ещё не привязан, сообщение не отправляется; результат попадает в
`/status` строкой «Автозапуск: вкл | не удалось (<причина>)» и в лог.

`/autostart on` использует ту же функцию и отвечает фактическим результатом,
а не желаемым: при неудаче — сообщение с причиной, и `settings.autostart`
всё равно сохраняется как `true`, чтобы попытка повторилась при следующем
запуске.

Тест `test/launch-agent.test.js` — на фейковом `app` c
`get/setLoginItemSettings`: уже включён, успешное включение, молчаливый отказ,
исключение, translocated-путь.
...
- `launch-agent.test.js` — уже включён, успешное включение, молчаливый отказ,
  исключение, translocated-путь.
