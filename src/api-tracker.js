...
// Локальный десктоп-агент TS (официальное приложение, с которым дашборд
// спаривает сессию браузера). Страница делает GET /pair-info здесь и POST'ит
// {machineId, token} в /api/v1/agent/pair; мы повторяем те же два вызова
// нативно, поэтому ни браузер, ни промпт «wants to access other apps» не
// нужны. Используем РЕАЛЬНЫЕ {machineId, token} агента — ничего не подделываем.
const AGENT_PAIR_INFO_URL = "http://127.0.0.1:47836/pair-info";
const AGENT_PAIR_INFO_TIMEOUT_MS = 1500;
...
function createApiTracker(opts) {
	...
	async function startOAuth() {
		...
		if (!location)
			throw new Error(`OAuth start failed (status ${response.status})`);
	...
	async function loginToGitea(state, email, password) {
		...
		if (redirectUrl) {
			homeUrl = redirectUrl.startsWith("http")
				? redirectUrl
				: `${GITEA_URL}${redirectUrl}`;
		}
		await makeRequest(homeUrl);

		const oauthUrl =
			`${GITEA_URL}/login/oauth/authorize?client_id=${OAUTH_CLIENT_ID}` +
		...
		if (!callbackUrl)
			throw new Error(
				`OAuth authorize failed (status ${oauthResponse.status})`,
			);

		// Manually follow redirects so every Set-Cookie is captured.
		let currentUrl = callbackUrl.startsWith("/")
	...
	async function ensureStarted() {
		...
		const response = await makeRequest(
			`${dashboardUrl}/api/v1/tracking/start`,
			{
				method: "POST",
		...
		if (response.status !== 200) {
			throw new Error(
				`start failed (${response.status}): ${(response.data && response.data.error) || "unknown"}`,
			);
		}
		const returned =
			(response.data &&
				response.data.tracking &&
				response.data.tracking.deviceId) ||
			deviceId;
		deviceId = returned;
		active = true;
	}
	...
	async function heartbeat() {
		const response = await makeRequest(
			`${dashboardUrl}/api/v1/tracking/heartbeat`,
			{
				method: "POST",
				headers: { "Content-Type": "application/json" },
		...
		let todaySeconds =
			data.todaySeconds != null ? data.todaySeconds : tracking.todaySeconds;
		let weekSeconds =
			data.weekSeconds != null ? data.weekSeconds : tracking.weekSeconds;
	...
	// Спарить эту сессию с локальным TS-агентом (127.0.0.1:47836). Дашборд
	// делает те же два вызова из браузера (что и вызывает промпт «wants to
	// access other apps and services»); нативный процесс ходит на localhost
	// без этого браузерного гейта. Берём у агента его РЕАЛЬНЫЕ {machineId,
	// token} — контроль целостности школы (доверенный агент) остаётся нетронут.
	async function pairAgent() {
		// 1) Читаем {machineId, token} у локального агента. Прямой axios-вызов —
		//    НЕ через makeRequest, иначе кука сессии дашборда улетит на
		//    кросс-оригин localhost-запрос.
		let info;
		try {
			const resp = await request({
				url: AGENT_PAIR_INFO_URL,
				timeout: AGENT_PAIR_INFO_TIMEOUT_MS,
				headers: { "User-Agent": USER_AGENT },
				validateStatus: () => true,
			});
			if (!resp || resp.status !== 200)
				return { paired: false, reason: "agent-unreachable" };
			info = resp.data;
		...
		const machineId =
			info && typeof info.machineId === "string" ? info.machineId : null;
		const token =
			info && typeof info.token === "string" ? info.token : undefined;
		if (!machineId) return { paired: false, reason: "no-machine-id" };
		// 2) Линкуем сессию с машиной. Нужны кука сессии + csrf-токен —
		//    makeRequest их подставляет.
		const resp = await makeRequest(`${dashboardUrl}/api/v1/agent/pair`, {
			method: "POST",
			headers: { "Content-Type": "application/json" },
			data: { machineId, token },
		});
		if (resp.status !== 200)
			return { paired: false, reason: `pair-failed-${resp.status}` };
		return { paired: true, machineId };
	}
